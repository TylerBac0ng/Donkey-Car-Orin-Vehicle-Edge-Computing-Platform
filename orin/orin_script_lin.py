...
# Updated: control channel changed from TCP -> UDP (video remains UDP). :contentReference[oaicite:0]{index=0}
...
class PiConnection:
    def __init__(self, pi_id: str, ip: str, UDP_PORT: int, TCP_PORT: int,
        ...
        # Keeping the name TCP_PORT to minimize edits; it now represents the Pi's UDP *control* port.
    ...
    def start(self):
        # No connect() needed for UDP control socket
...
PI_CONFIGS = {
    ...
    # Add Pis here, include ip address and the UDP_PORT + control port (stored in TCP_PORT) each pi will be occupying
}
